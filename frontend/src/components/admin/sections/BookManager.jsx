...
const BookManager = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Отправляем данные книги как JSON с явным типом 'application/json'
      formDataToSend.append('book', new Blob([JSON.stringify(formData)], { type: 'application/json' }));
      if (selectedFile) {
      ...
      if (currentBook) {
        // Обновление книги
        await axios.put(`${API_URL}/rest/books/${currentBook.id}`, formDataToSend, {
          withCredentials: true
        });
        toast.success('Книга успешно обновлена');
      } else {
        // Создание новой книги
        await axios.post(`${API_URL}/rest/books`, formDataToSend, {
          withCredentials: true
        });
                  ...
                  <div className="mb-4">
                    ...
                    <div {...getRootProps()} className={`dropzone ${isDragActive ? 'active' : ''}`}>
                      ...
                      {formData.coverImageUrl && (
                          <img src={formData.coverImageUrl} alt="Preview" style={{ marginTop: '10px', maxWidth: '100px' }} />
                      )}
                    </div>
                  </div>
