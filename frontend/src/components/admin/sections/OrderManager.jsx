  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      const response = await axios.put(
        `${API_URL}/rest/orders/${orderId}/status?status=${newStatus}`,
        {},
        { withCredentials: true }
      );
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{order.createdBy || 'Неизвестно'}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{order.createdWhen}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{order.total || '0'} ₽</td>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Детали заказа #{selectedOrder.id}</h3>
              <button
            ...
            <div className="mb-4">
              <p><strong>Пользователь:</strong> {selectedOrder.createdBy || 'Неизвестно'}</p>
              <p><strong>Дата создания:</strong> {selectedOrder.createdWhen}</p>
              <p><strong>Статус:</strong> {selectedOrder.orderStatus || 'Не указан'}</p>
              <p><strong>Общая сумма:</strong> {selectedOrder.total?.source || '0'} ₽</p>
            </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {selectedOrder.products?.map((product, index) => (
                    <tr key={index}>
                      <td className="px-4 py-2 whitespace-nowrap">{product.name || 'Неизвестно'}</td>
                      <td className="px-4 py-2 whitespace-nowrap">1</td>
                      <td className="px-4 py-2 whitespace-nowrap">{product.price?.source || '0'} ₽</td>
                      <td className="px-4 py-2 whitespace-nowrap">{product.price?.source || '0'} ₽</td>
                    </tr>
                  ))}
                </tbody>
